...
class JamaahHajiController extends Controller
{
    public function index()
    {
        $haji = JamaahHaji::all();
        return view('haji.index', compact('haji'));
    }

    public function create()
    {
        // Ambil semua data peserta wisata luar negeri untuk ditampilkan sebagai "data hubungan"
        $jamaahhaji = JamaahHaji::all();
        // Kirim ke view
        return view('haji.create', compact('jamaahhaji'));
    }

    public function show($id)
    {
        $data = JamaahHaji::where('id', $id)->get(); // tetap pakai get() agar bisa foreach, atau pakai ->first() kalau hanya satu
        return view('datajhview', ['haji' => $data]);
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'nik' => 'required|string|max:20|unique:jamaah_haji,nik',
            'tempat_lahir' => 'required|string|max:100',
            'tanggal_lahir' => 'required|date',
            'jenis_kelamin' => 'required|in:L,P',
            'foto_peserta' => 'required|image|mimes:jpeg,png,jpg,gif|max:2048',
            'no_paspor' => 'required|string|max:50|unique:jamaah_haji,no_paspor',
    ...
    {
        $peserta = JamaahHaji::findOrFail($id);
        return view('datajhchange', compact('peserta'));
    }
